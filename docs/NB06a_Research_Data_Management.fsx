...
(**
The NFDI (Nationale Forschungsdateninfrastruktur) is a german infrastructure that targets standardization of RDM. The CSB group is part of DataPLANT, a consortium within the NFDI, which tackles the field of basic plant research. To achieve the already introduced FAIR principles, DataPLANT envisioned the **Annotated Research Context** (short: **ARC**\).
...
- the sink columns
  - **Sample Name**  
    Sample names describe the name of the _output_ of a process. For example, if a process ends in the specimen being stored in an eppendorfer tube, it could be the name of this eppi.
  - **Data File Name**  
...
![](ParameterBuildingBlock_filled.png)  
***Figure 11: Building Block "Parameter [instrument model]", filled with values.*** 

Ideally, an annotation table describes this experiment's process chronologically so that a researcher trying to replicate it can easily do so. The worksheets an _Assay_ file consists of, should be ordere chronologically, too.  

While you could do all this stuff by hand, it is much easier of you use a tool that automatizes much of the work. For this, an MS Excel add-in exists that facilitates the work with Xlsx files massively.
...
![](CreateAnnotationTable.jpg)
4. [Add Building Blocks](https://github.com/nfdi4plants/Swate/wiki/Docs03-Building-Blocks)  
![](AddBuildingBlock.jpg)  
Search the term database (B) for a term fitting. If you don't find one, just write a custom term that suits your needs.  
You can add units to them before creation (D) or afterwards:  
![](UpdateUnit.jpg)
5. [Fill cells with terms](https://github.com/nfdi4plants/Swate/wiki/docs04-Ontology-Term-Search)  
![](TermInsert.jpg)  
...
6. You can also type values into cells without using the Ontology Term Search, but don't forget to update them afterwards via the _Update Ontology Terms_ function:  
![](UpdateOntologyTerms.png)
